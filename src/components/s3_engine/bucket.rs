    ...
    Mt: S3MountBucket<S3EngineImplError> + Send + Sync,
{
    async fn make_bucket(&self, bucket: &str, region: Option<&str>, features: BucketFeatures) -> Result<S3Bucket, S3EngineImplError> {
        if self.metadata.load_bucket(bucket).await?.is_some() {
            return Err(S3EngineImplError::BucketAlreadyExists(bucket.to_owned()));
        }
    ...
    async fn head_bucket(&self, bucket: &str) -> Result<S3Bucket, S3EngineImplError> {
        self.metadata.load_bucket(bucket).await?
            .ok_or_else(|| S3EngineImplError::BucketNotFound(bucket.to_owned()))
    }

    async fn get_bucket(&self, bucket: &str) -> Result<S3Bucket, S3EngineImplError> {
    ...
    async fn delete_bucket(&self, bucket: &str, force: bool) -> Result<(), S3EngineImplError> {
        self.head_bucket(bucket).await?;
        ...
        if !page.objects.is_empty() && !force {
            return Err(S3EngineImplError::BucketNotEmpty(bucket.to_owned()));
        }
        if force {
    ...
    async fn list_objects_v1(&self, bucket: &str, options: ListOptions) -> Result<ObjectListPage, S3EngineImplError> {
        self.head_bucket(bucket).await?;
        self.metadata.list_objects(bucket, &options).await
    }

    async fn list_objects_v2(&self, bucket: &str, options: ListOptions) -> Result<ObjectListPage, S3EngineImplError> {
        self.head_bucket(bucket).await?;
        self.metadata.list_objects(bucket, &options).await
    }

    async fn list_object_versions(&self, bucket: &str, options: ListOptions) -> Result<ObjectListPage, S3EngineImplError> {
        self.head_bucket(bucket).await?;
        self.metadata.list_objects(bucket, &options).await
    }
}
