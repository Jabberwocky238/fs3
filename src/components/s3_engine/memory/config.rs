...
impl S3BucketConfigEngine for MemoryS3Engine {
    ...
    async fn put_bucket_policy(&self, bucket: &str, policy_json: String) -> Result<(), Self::Error> {
    ...
    async fn put_bucket_lifecycle(&self, bucket: &str, lifecycle_xml: String) -> Result<(), Self::Error> {
        let mut state = self.state.write().await;
        state.ensure_bucket(bucket)?;
    ...
    async fn put_bucket_encryption(&self, bucket: &str, encryption_xml: String) -> Result<(), Self::Error> {
        let mut state = self.state.write().await;
        state.ensure_bucket(bucket)?;
    ...
    async fn get_bucket_object_lock_config(&self, bucket: &str) -> Result<Option<TimedDocument>, Self::Error> {
        let state = self.state.read().await;
        state.ensure_bucket(bucket)?;
        Ok(state.bucket_metadata.get(bucket).and_then(|m| m.object_lock_xml.clone()))
    }

    async fn put_bucket_object_lock_config(&self, bucket: &str, object_lock_xml: String) -> Result<(), Self::Error> {
        let mut state = self.state.write().await;
        state.ensure_bucket(bucket)?;
    ...
    async fn put_bucket_versioning(&self, bucket: &str, versioning_xml: String) -> Result<(), Self::Error> {
        let mut state = self.state.write().await;
        state.ensure_bucket(bucket)?;
    ...
    async fn put_bucket_notification(&self, bucket: &str, notification_xml: String) -> Result<(), Self::Error> {
        let mut state = self.state.write().await;
        state.ensure_bucket(bucket)?;
    ...
    async fn put_bucket_replication(&self, bucket: &str, replication_xml: String) -> Result<(), Self::Error> {
        let mut state = self.state.write().await;
        state.ensure_bucket(bucket)?;
    ...
    async fn put_bucket_tagging(&self, bucket: &str, tagging_xml: String) -> Result<(), Self::Error> {
        let mut state = self.state.write().await;
        state.ensure_bucket(bucket)?;
