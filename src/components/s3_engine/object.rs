        ...
        + Send + Sync,
{
    ...
    async fn get_object_tagging(&self, bucket: &str, key: &str) -> Result<TagMap, S3EngineImplError> {
        Ok(self.head_object(bucket, key, ObjectReadOptions::default()).await?.user_tags)
    }

    async fn put_object_tagging(&self, bucket: &str, key: &str, tags: TagMap) -> Result<(), S3EngineImplError> {
        let mut obj = self.head_object(bucket, key, ObjectReadOptions::default()).await?;
        obj.user_tags = tags;
        self.metadata.store_object_meta(&obj).await
    }

    async fn delete_object_tagging(&self, bucket: &str, key: &str) -> Result<(), S3EngineImplError> {
        self.put_object_tagging(bucket, key, TagMap::new()).await
    }

    async fn get_object_retention(&self, bucket: &str, key: &str) -> Result<Option<ObjectRetention>, S3EngineImplError> {
        Ok(self.head_object(bucket, key, ObjectReadOptions::default()).await?.retention)
    }

    async fn put_object_retention(&self, bucket: &str, key: &str, retention: ObjectRetention) -> Result<(), S3EngineImplError> {
        let mut obj = self.head_object(bucket, key, ObjectReadOptions::default()).await?;
        obj.retention = Some(retention);
        self.metadata.store_object_meta(&obj).await
    }

    async fn get_object_legal_hold(&self, bucket: &str, key: &str) -> Result<Option<ObjectLegalHold>, S3EngineImplError> {
        Ok(self.head_object(bucket, key, ObjectReadOptions::default()).await?.legal_hold)
    }

    async fn put_object_legal_hold(&self, bucket: &str, key: &str, legal_hold: ObjectLegalHold) -> Result<(), S3EngineImplError> {
        let mut obj = self.head_object(bucket, key, ObjectReadOptions::default()).await?;
