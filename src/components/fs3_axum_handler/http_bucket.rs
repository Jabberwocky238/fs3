    ...
    E: std::fmt::Display + From<S3HandlerBridgeError> + From<S3EngineError> + 'static,
{
    ...
    let resp = match method {
        ...
        Method::GET if has(&q, "uploads") => S3Response::ListMultipartUploads(handler.list_multipart_uploads(ListMultipartUploadsRequest { bucket: mk(), query: list_q.clone() }).await.map_err(bucket_err)?),
        Method::GET if get(&q, "list-type").as_deref() == Some("2") && get(&q, "metadata").as_deref() == Some("true") => S3Response::ListObjectsV2M(handler.list_objects_v2m(ListObjectsV2MRequest { bucket: mk(), query: list_q.clone(), metadata: true }).await.map_err(bucket_err)?),
        Method::GET if get(&q, "list-type").as_deref() == Some("2") => S3Response::ListObjectsV2(handler.list_objects_v2(ListObjectsV2Request { bucket: mk(), query: list_q.clone() }).await.map_err(bucket_err)?),
        Method::GET if has(&q, "versions") && get(&q, "metadata").as_deref() == Some("true") => S3Response::ListObjectVersionsM(handler.list_object_versions_m(ListObjectVersionsMRequest { bucket: mk(), query: list_q.clone(), metadata: true }).await.map_err(bucket_err)?),
        Method::GET if has(&q, "versions") => S3Response::ListObjectVersions(handler.list_object_versions(ListObjectVersionsRequest { bucket: mk(), query: list_q.clone() }).await.map_err(bucket_err)?),
        ...
        Method::GET => S3Response::ListObjectsV1(handler.list_objects_v1(ListObjectsV1Request { bucket: mk(), query: list_q.clone() }).await.map_err(bucket_err)?),

        Method::PUT if has(&q, "lifecycle") => S3Response::PutBucketLifecycle(handler.put_bucket_lifecycle(PutBucketLifecycleRequest { bucket: mk(), xml: text.clone() }).await.map_err(bucket_err)?),
        Method::PUT if has(&q, "replication") => S3Response::PutBucketReplicationConfig(handler.put_bucket_replication_config(PutBucketReplicationConfigRequest { bucket: mk(), xml: text.clone() }).await.map_err(bucket_err)?),
        Method::PUT if has(&q, "encryption") => S3Response::PutBucketEncryption(handler.put_bucket_encryption(PutBucketEncryptionRequest { bucket: mk(), xml: text.clone() }).await.map_err(bucket_err)?),
        Method::PUT if has(&q, "policy") => S3Response::PutBucketPolicy(handler.put_bucket_policy(PutBucketPolicyRequest { bucket: mk(), json: text.clone() }).await.map_err(bucket_err)?),
        Method::PUT if has(&q, "object-lock") => S3Response::PutBucketObjectLockConfig(handler.put_bucket_object_lock_config(PutBucketObjectLockConfigRequest { bucket: mk(), xml: text.clone() }).await.map_err(bucket_err)?),
        Method::PUT if has(&q, "tagging") => S3Response::PutBucketTagging(handler.put_bucket_tagging(PutBucketTaggingRequest { bucket: mk(), xml: text.clone() }).await.map_err(bucket_err)?),
        Method::PUT if has(&q, "versioning") => S3Response::PutBucketVersioning(handler.put_bucket_versioning(PutBucketVersioningRequest { bucket: mk(), xml: text.clone() }).await.map_err(bucket_err)?),
        Method::PUT if has(&q, "notification") => S3Response::PutBucketNotification(handler.put_bucket_notification(PutBucketNotificationRequest { bucket: mk(), xml: text.clone() }).await.map_err(bucket_err)?),
        ...
        // Method::POST => S3Response::PostPolicy(handler.post_policy(PostPolicyRequest { bucket: mk(), form: PostPolicyForm { fields: HashMap::new(), payload: body.to_vec() } }).await.map_err(bucket_err)?),
