...
{
    Router::new()
        .route("/{bucket}/{*object}", any(object_entry::<T>))
        .with_state(state)
}
    ...
    T: S3Handler + Send + Sync,
{
    ...
    let resp = match method {
                        ...
                        .map(|v| {
                            v.split(',')
                                ...
                                .filter_map(|x| match x.as_str() {
                                    ...
                                    "objectparts" | "object_parts" | "parts" => {
                                        Some(ObjectAttribute::ObjectParts)
                                    }
                                    "storageclass" | "storage_class" => {
                                        Some(ObjectAttribute::StorageClass)
                                    }
                                    "objectsize" | "object_size" | "size" => {
                                        Some(ObjectAttribute::ObjectSize)
                                    }
                                    "lastmodified" | "last_modified" => {
                                        Some(ObjectAttribute::LastModified)
                                    }
                                    _ => None,
                                })
                                .collect()
        ...
        Method::GET if has(&q, "uploadId") => S3Response::ListObjectParts(
            handler
                .list_object_parts(ListObjectPartsRequest {
                    object: mk(),
        ...
        Method::GET if has(&q, "acl") => S3Response::GetObjectAcl(
            handler
                .get_object_acl(GetObjectAclRequest { object: mk() })
                .await?,
        ),
        Method::GET if has(&q, "lambdaArn") => S3Response::GetObjectLambda(
            handler
                .get_object_lambda(GetObjectLambdaRequest {
                    object: mk(),
        ...
        Method::PUT if has(&q, "uploadId") && has(&q, "partNumber") && copy_source.is_some() => {
            S3Response::CopyObjectPart(
                handler
                    .copy_object_part(CopyObjectPartRequest {
                        object: mk(),
        ...
        Method::PUT if has(&q, "uploadId") && has(&q, "partNumber") => S3Response::PutObjectPart(
            handler
                .put_object_part(PutObjectPartRequest {
                    object: mk(),
                    multipart: multipart_selector(&q),
        ...
        Method::PUT if has(&q, "acl") => {
            ...
            S3Response::PutObjectAcl(
                handler
                    .put_object_acl(PutObjectAclRequest { object: mk(), acl })
                    .await?,
            )
        }
        Method::PUT if header_eq(&headers, "x-amz-snowball-extract", "true") => {
            S3Response::PutObjectExtract(
                handler
                    .put_object_extract(PutObjectExtractRequest {
                        object: mk(),
                        body: body_stream(body),
                    })
                    .await?,
        ...
        Method::PUT if header(&headers, "x-amz-write-offset-bytes").is_some() => {
            S3Response::AppendObjectRejected(
                handler
                    .append_object_rejected(AppendObjectRejectedRequest {
                        object: mk(),
                        write_offset_bytes: header(&headers, "x-amz-write-offset-bytes")
                            .unwrap_or_default(),
                        body: body_stream(body),
                    })
                    .await?,
        ...
        Method::PUT => {
            let content_length =
                header(&headers, "content-length").and_then(|v| v.parse::<u64>().ok());
            let mut user_metadata = HashMap::new();
            for (k, v) in headers.iter() {
                if let Some(key) = k.as_str().strip_prefix("x-amz-meta-") {
                    if let Ok(val) = v.to_str() {
            ...
            S3Response::PutObject(
                handler
                    .put_object(PutObjectRequest {
                        ...
                        sse_customer_key: header(
                            &headers,
                            "x-amz-server-side-encryption-customer-key",
                        ),
                        sse_customer_key_md5: header(
                            &headers,
                            "x-amz-server-side-encryption-customer-key-md5",
                        ),
                        sse_kms_key_id: header(
                            &headers,
        ...
        Method::POST if has(&q, "uploads") => S3Response::NewMultipartUpload(
            handler
                .new_multipart_upload(NewMultipartUploadRequest { object: mk() })
                .await?,
        ),
        ...
        Method::POST if has(&q, "restore") => {
            ...
            S3Response::PostRestoreObject(
                handler
                    .post_restore_object(PostRestoreObjectRequest {
                        object: mk(),
                        restore,
                    })
                    .await?,
