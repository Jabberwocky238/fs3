...
use crate::types::{
    errors::FS3Error,
    s3::request::{EventFilter, ListQuery, MultipartSelector},
};

pub fn has(q: &HashMap<String, String>, key: &str) -> bool {
    q.contains_key(key)
}
pub fn get(q: &HashMap<String, String>, key: &str) -> Option<String> {
    q.get(key).cloned()
}
...
pub fn body_stream(body: Body) -> crate::types::s3::core::BoxByteStream {
    Box::pin(
        body.into_data_stream()
            .map(|result: Result<axum::body::Bytes, axum::Error>| {
...
pub async fn body_text(body: Body) -> Result<String, FS3Error> {
    let chunks = body_stream(body).collect::<Vec<_>>().await;
    let mut buf = Vec::new();
    for chunk in chunks {
        let bytes = chunk.map_err(|e| FS3Error::from(format!("stream error: {e}")))?;
        buf.extend_from_slice(&bytes);
    }
    String::from_utf8(buf).map_err(|e| FS3Error::from(format!("invalid utf-8 body: {e}")))
}
