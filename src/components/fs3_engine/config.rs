...
impl S3BucketVersionEngine<S3EngineError> for FS3Engine {
    async fn get_bucket_versioning(
        ...
        Ok(status.map(|s| BucketVersioning {
            status: s,
            mfa_delete: None,
        }))
    }
    ...
    async fn put_bucket_versioning(
        ...
        bucket: &str,
        status: String,
        _mfa_delete: Option<String>,
    ) -> Result<(), S3EngineError> {
        let ctx = crate::types::s3::object_layer_types::Context {
            request_id: "".to_string(),
        };
        self.storage
            .write_bucket_versioning(&ctx, bucket, &status)
            .await
    }
}
...
impl S3BucketConfigEngine<S3EngineError> for FS3Engine {
    ...
    async fn put_bucket_metadata(
        &self,
        _bucket: &str,
        _metadata: BucketMetadataBundle,
    ) -> Result<(), S3EngineError> {
        Ok(())
    }
}
