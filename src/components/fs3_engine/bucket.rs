...
impl S3BucketEngine<FS3Error> for FS3Engine {
    async fn make_bucket(
        ...
        bucket: &str,
        _region: Option<&str>,
        _features: BucketFeatures,
    ) -> Result<S3Bucket, FS3Error> {
        let ctx = crate::types::s3::object_layer_types::Context {
            request_id: "".to_string(),
        };
        self.object_layer
            .make_bucket(&ctx, bucket, Default::default())
            .await?;
        Ok(S3Bucket {
    ...
    async fn get_bucket(&self, bucket: &str) -> Result<S3Bucket, FS3Error> {
        ...
        let info = self
            .object_layer
            .get_bucket_info(&ctx, bucket, Default::default())
            .await?;
        Ok(S3Bucket {
            identity: BucketIdentity {
                name: info.name,
    ...
    async fn list_buckets(&self) -> Result<Vec<S3Bucket>, FS3Error> {
        ...
        let buckets = self
            .object_layer
            .list_buckets(&ctx, Default::default())
            .await?;
        Ok(buckets
            .into_iter()
            .map(|b| S3Bucket {
    ...
    ) -> Result<ObjectListPage, FS3Error> {
    ...
    async fn list_objects_v2(
        ...
        if let Ok(entries) = std::fs::read_dir(&bucket_path) {
            for entry in entries.flatten() {
                ...
                if let Ok(info) = self
                    .object_layer
                    .get_object_info(&ctx, bucket, &object_name, Default::default())
                    .await
                {
