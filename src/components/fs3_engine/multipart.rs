...
impl S3MultipartEngine<S3EngineError> for FS3Engine {
    async fn new_multipart_upload(
        ...
        let result = self
            .object_layer
            .new_multipart_upload(&ctx, bucket, key, Default::default())
            .await?;
        Ok(MultipartUpload {
    ...
    async fn complete_multipart_upload(
        ...
        let parts = completed
            .parts
            .into_iter()
            .map(|p| crate::types::s3::storage_types::CompletePart {
        ...
        let result = self
            .object_layer
            .complete_multipart_upload(&ctx, bucket, key, upload_id, parts, Default::default())
            .await?;
        Ok(S3Object {
            bucket: bucket.to_string(),
            key: key.to_string(),
