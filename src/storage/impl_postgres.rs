...
impl PostgresStore {
    pub async fn new(dsn: String, seed: StorageSnapshot) -> Result<Self, StorageError> {
        if dsn.trim().is_empty() {
            return Err(StorageError::Io("postgres dsn is empty".to_string()));
        }
    ...
    async fn ensure_tables(&self) -> Result<(), StorageError> {
        sqlx::query("CREATE TABLE IF NOT EXISTS users (user_id TEXT PRIMARY KEY, enabled BOOLEAN NOT NULL, groups_json JSONB NOT NULL, attrs_json JSONB NOT NULL)")
            .execute(&self.pool)
            .await
            .map_err(|e| StorageError::Db(e.to_string()))?;
        let _ = sqlx::query(
            "ALTER TABLE users ADD COLUMN IF NOT EXISTS access_key TEXT NOT NULL DEFAULT ''",
        )
        .execute(&self.pool)
        .await;
        let _ = sqlx::query(
            "ALTER TABLE users ADD COLUMN IF NOT EXISTS secret_key TEXT NOT NULL DEFAULT ''",
        )
        .execute(&self.pool)
        .await;
        sqlx::query("CREATE TABLE IF NOT EXISTS policy_groups (name TEXT PRIMARY KEY, enabled BOOLEAN NOT NULL, users_json JSONB NOT NULL, policies_json JSONB NOT NULL)")
            .execute(&self.pool)
            .await
            .map_err(|e| StorageError::Db(e.to_string()))?;
        ...
        let _ = sqlx::query("ALTER TABLE policy_groups RENAME COLUMN statements_json TO policies_json")
            .execute(&self.pool)
            .await;
        sqlx::query("CREATE TABLE IF NOT EXISTS bucket_metadata (bucket TEXT PRIMARY KEY, owner TEXT NOT NULL, labels_json JSONB NOT NULL, attrs_json JSONB NOT NULL)")
            .execute(&self.pool)
            .await
            .map_err(|e| StorageError::Db(e.to_string()))?;
        Ok(())
...
impl UserStore for PostgresStore {
    async fn list_users(&self) -> Result<Vec<UserRecord>, StorageError> {
        let rows = sqlx::query_as::<_, (String, bool, String, String, String, String)>(
            "SELECT user_id, enabled, groups_json::text, attrs_json::text, access_key, secret_key FROM users",
        )
        .fetch_all(&self.pool)
        .await
        .map_err(|e| StorageError::Db(e.to_string()))?;
        ...
        for u in &users {
            ...
            if changed {
                ...
                let attrs_json = serde_json::to_string(&u.attrs)
                    .map_err(|e| StorageError::Serde(e.to_string()))?;
                let res = sqlx::query("INSERT INTO users(user_id, enabled, groups_json, attrs_json, access_key, secret_key) VALUES($1,$2,$3::jsonb,$4::jsonb,$5,$6) ON CONFLICT(user_id) DO UPDATE SET enabled=EXCLUDED.enabled, groups_json=EXCLUDED.groups_json, attrs_json=EXCLUDED.attrs_json, access_key=EXCLUDED.access_key, secret_key=EXCLUDED.secret_key")
                    .bind(&u.user_id)
                    .bind(u.enabled)
                    .bind(groups_json)
                    .bind(attrs_json)
...
impl PolicyStore for PostgresStore {
    async fn list_policy_groups(&self) -> Result<Vec<PolicyGroup>, StorageError> {
        let rows = sqlx::query_as::<_, (String, String, String)>(
            "SELECT name, users_json::text, policies_json::text FROM policy_groups",
        )
        .fetch_all(&self.pool)
        .await
        .map_err(|e| StorageError::Db(e.to_string()))?;
    ...
    async fn save_policy_groups(&self, groups: Vec<PolicyGroup>) -> Result<(), StorageError> {
        ...
        for g in &groups {
            ...
            if changed {
                ...
                let policies_json = serde_json::to_string(&g.policies)
                    .map_err(|e| StorageError::Serde(e.to_string()))?;
                let res = sqlx::query("INSERT INTO policy_groups(name, users_json, policies_json) VALUES($1,$2::jsonb,$3::jsonb) ON CONFLICT(name) DO UPDATE SET users_json=EXCLUDED.users_json, policies_json=EXCLUDED.policies_json")
                    .bind(&g.name)
                    .bind(users_json)
                    .bind(policies_json)
                    .execute(&self.pool)
        ...
        for old in current_map.keys() {
            if !new_ids.contains(old) {
                let res = sqlx::query("DELETE FROM policy_groups WHERE name = $1")
                    .bind(old)
                    .execute(&self.pool)
                    .await
                    .map_err(|e| StorageError::Db(e.to_string()))?;
...
#[async_trait::async_trait]
impl BucketMetaStore for PostgresStore {
    async fn list_bucket_metadata(&self) -> Result<Vec<BucketMetadata>, StorageError> {
        let rows = sqlx::query_as::<_, (String, String, String, String)>(
            "SELECT bucket, owner, labels_json::text, attrs_json::text FROM bucket_metadata",
        )
        .fetch_all(&self.pool)
        .await
        .map_err(|e| StorageError::Db(e.to_string()))?;
    ...
    async fn upsert_bucket_metadata(&self, meta: BucketMetadata) -> Result<(), StorageError> {
        ...
        let res = sqlx::query("INSERT INTO bucket_metadata(bucket, owner, labels_json, attrs_json) VALUES($1,$2,$3::jsonb,$4::jsonb) ON CONFLICT(bucket) DO UPDATE SET owner=EXCLUDED.owner, labels_json=EXCLUDED.labels_json, attrs_json=EXCLUDED.attrs_json")
            .bind(&meta.bucket)
            .bind(&meta.owner)
            .bind(labels_json)
            .bind(attrs_json)
