    ...
    fn from(resp: &S3Response) -> Self {
        match resp {
            ...
            S3Response::GetBucketLifecycle(r) => Some(XMLResponse {
                body: format!(
                    r#"<?xml version="1.0" encoding="UTF-8"?><LifecycleConfiguration xmlns="http://s3.amazonaws.com/doc/2006-03-01/">{}</LifecycleConfiguration>"#,
                    r.rules.iter().map(|rule| rule.as_str()).collect::<String>()
                ),
            }),
            S3Response::GetBucketEncryption(r) => Some(XMLResponse {
                body: format!(
                    r#"<?xml version="1.0" encoding="UTF-8"?><ServerSideEncryptionConfiguration xmlns="http://s3.amazonaws.com/doc/2006-03-01/"><Rule><ApplyServerSideEncryptionByDefault>{}{}</ApplyServerSideEncryptionByDefault></Rule></ServerSideEncryptionConfiguration>"#,
                    r.sse_algorithm
                        .as_ref()
                        .map(|a| format!("<SSEAlgorithm>{}</SSEAlgorithm>", a))
                        .unwrap_or_default(),
                    r.kms_master_key_id
                        .as_ref()
                        .map(|k| format!("<KMSMasterKeyID>{}</KMSMasterKeyID>", k))
                        .unwrap_or_default()
                ),
            }),
            ...
            S3Response::GetBucketReplicationConfig(r) => Some(XMLResponse {
                body: format!(
                    r#"<?xml version="1.0" encoding="UTF-8"?><ReplicationConfiguration xmlns="http://s3.amazonaws.com/doc/2006-03-01/">{}{}</ReplicationConfiguration>"#,
                    r.role
                        .as_ref()
                        .map(|ro| format!("<Role>{}</Role>", ro))
                        .unwrap_or_default(),
            ...
            S3Response::GetBucketWebsite(r) => Some(XMLResponse {
                body: format!(
                    ...
                    r.index_document
                        .as_ref()
                        .map(|s| format!("<IndexDocument><Suffix>{}</Suffix></IndexDocument>", s))
                        .unwrap_or_default(),
                    r.error_document
                        .as_ref()
                        .map(|s| format!("<ErrorDocument><Key>{}</Key></ErrorDocument>", s))
                        .unwrap_or_default()
                ),
            }),
