...
pub trait BucketTaggingS3Handler: Send + Sync {
    ...
    async fn get_bucket_tagging(&self, req: GetBucketTaggingRequest) -> Result<GetBucketTaggingResponse , FS3Error> {
    ...
    async fn put_bucket_tagging(&self, req: PutBucketTaggingRequest) -> Result<PutBucketTaggingResponse , FS3Error> {
    ...
    async fn delete_bucket_tagging(&self, req: DeleteBucketTaggingRequest) -> Result<DeleteBucketTaggingResponse , FS3Error> {
        check_access(self.bucket_tagging_policy_provider(), S3Action::PutBucketTagging, Some(&req.bucket.bucket), None).await?;
