...
use crate::types::traits::s3_handler::request_validation::checksum::{
    ChecksumAlgorithm, ChecksumExpectation, ContentMd5Expectation,
};

#[derive(Debug, Clone, Default)]
pub struct RequestValidationPlan {
...
pub struct ValidatingRequestStream {
    inner: BoxByteStream,
    md5: Option<(ContentMd5Expectation, md5::Context)>,
    checksum: Option<ChecksumState>,
    finished: bool,
    result: RequestValidationResult,
}

impl ValidatingRequestStream {
    pub fn new(inner: BoxByteStream, plan: RequestValidationPlan) -> Self {
        Self {
            inner,
            ...
            finished: false,
            result: RequestValidationResult::default(),
        }
    }
    ...
    pub fn validation_result(&self) -> &RequestValidationResult {
        &self.result
    }

    fn update_digests(&mut self, chunk: &Bytes) {
        if let Some((_, md5)) = &mut self.md5 {
    ...
    fn finalize(&mut self) -> Result<(), FS3Error> {
        ...
        if let Some((expected, md5)) = self.md5.take() {
            let digest = md5.finalize();
            let actual = STANDARD.encode(digest.0);
            if actual != expected.base64_value {
                return Err(FS3Error::bad_request("BadDigest"));
            }
            self.result.computed_content_md5_base64 = Some(actual);
        }

        if let Some(checksum) = self.checksum.take() {
            let (expected, actual) = checksum.finalize();
            if actual != expected.base64_value {
                return Err(FS3Error::bad_request("ChecksumMismatch"));
            }
