...
pub trait BucketReplicationS3Handler: Send + Sync {
    ...
    async fn get_bucket_replication_config(&self, req: GetBucketReplicationConfigRequest) -> Result<GetBucketReplicationConfigResponse , FS3Error> {
    ...
    async fn put_bucket_replication_config(&self, req: PutBucketReplicationConfigRequest) -> Result<PutBucketReplicationConfigResponse , FS3Error> {
    ...
    async fn delete_bucket_replication(&self, req: DeleteBucketReplicationRequest) -> Result<DeleteBucketReplicationResponse , FS3Error> {
        check_access(self.bucket_replication_policy_provider(), S3Action::PutReplicationConfiguration, Some(&req.bucket.bucket), None).await?;
        self.bucket_replication_engine_provider().delete_bucket_replication(&req.bucket.bucket).await?;
        Ok(Default::default())
    }

    async fn get_bucket_replication_metrics_v2(&self, req: GetBucketReplicationMetricsV2Request) -> Result<GetBucketReplicationMetricsV2Response , FS3Error> {
    ...
    async fn get_bucket_replication_metrics(&self, req: GetBucketReplicationMetricsRequest) -> Result<GetBucketReplicationMetricsResponse , FS3Error> {
        let _r = self.bucket_replication_engine_provider().get_bucket_replication_metrics(&req.bucket.bucket).await?;
        Ok(GetBucketReplicationMetricsResponse { ..Default::default() })
    }

    async fn validate_bucket_replication_creds(&self, req: ValidateBucketReplicationCredsRequest) -> Result<ValidateBucketReplicationCredsResponse , FS3Error> {
