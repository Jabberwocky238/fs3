...
#[async_trait]
pub trait ObjectRetentionS3Handler<E: Error + Send + Sync + 'static>: Send + Sync {
    ...
    async fn put_object_retention(&self, req: PutObjectRetentionRequest) -> Result<PutObjectRetentionResponse, E> {
        ...
        self.object_retention_engine_provider().put_object_retention(&req.bucket.bucket, &req.object.object, req.retention).await?;
        Ok(Default::default())
    }
}
