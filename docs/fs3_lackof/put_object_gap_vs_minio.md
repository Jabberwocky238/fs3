...
## 3.1 流式入口已修复，并开始接入校验链，但仍未闭环

虽然 PutObject 入口已经变成流式 body，而且 `s3_handler` 内部已经开始接入请求验证模块，但 MinIO `PutObjectHandler` 上层的大量流式语义当前仍未完整实现。

当前已经具备的部分：

1. `Content-MD5` 流式校验骨架
2. `x-amz-checksum-*` 流式校验骨架
3. `aws-chunked` 请求头解析
4. `aws-chunked` 正文解包骨架
5. `x-amz-trailer` 解析骨架
6. SSE-S3 / SSE-KMS / SSE-C 请求头组合校验骨架

但距离 MinIO 的真实链路仍然还有明显缺口：

1. `aws-chunked` 每个 chunk 的 streaming signature 校验未实现
2. trailer checksum 解析后尚未与计算值比对
3. SSE-C key 与 key-md5 的真实性校验未实现
4. SSE-S3 / SSE-KMS / SSE-C 的真正加解密未实现
5. checksum / md5 结果尚未稳定回写到对象元数据和响应

因此，当前更准确的状态不是“完全没有流式校验链”，而是：

1. 请求入口和验证骨架已经出现
2. 但 MinIO 意义上的完整流式校验链仍未闭环
...
## 3.4 ErasureServerPools 已开始实现 erasure/shard/bitrot/quorum 骨架，但仍与 MinIO 不兼容

当前 `ErasureServerPools::put_object()` 已不再只是“单文件写入再提交”。

当前已经开始具备的部分：

1. 对象数据先被收集后再切成多个 shard
2. shard 以独立文件形式写入临时目录
3. 每个 shard 有一层 bitrot-like frame
4. 写入路径中已经出现最小化的 write quorum 判断
5. `xl.meta` 中已经开始写入 `erasure_m / erasure_n`

但它仍然不是 MinIO 意义上的 `erasureServerPools`，核心原因是当前实现只是“fs3 自己的骨架”，而不是“MinIO-compatible erasure layout”。

当前仍然缺少或不兼容的点包括：

1. 没有 pool 选择
2. 没有多磁盘/多节点分布
3. 写盘布局不是 MinIO 的真实布局
4. bitrot 写入格式不是 MinIO 的真实格式
5. quorum 只是本地简化计数，不是 MinIO 的多盘 quorum 语义
6. 读路径尚未实现缺 shard / 坏 shard 恢复

因此，当前状态应描述为：

1. erasure/shard/bitrot/quorum 已经开始进入主链
2. 但它们还没有实现成 MinIO 可兼容的存储语义

## 3.5 存储层已经出现 shard 写入，但底层落盘语义仍未对齐 MinIO

`XlStorage::create_file()` 本身仍然只是：

1. 创建父目录
2. 创建文件
3. 顺序写入 stream

真正的 shard 拆分和 bitrot-like frame 目前是在 object layer 写路径里完成，然后再把每个 shard 作为普通流交给 `create_file()`。

这意味着 fs3 当前虽然不再是“单文件 + rename 提交”的旧状态，但它与 MinIO 的真实底层落盘语义仍然存在关键差距：

1. shard writer 不是 MinIO 的真实 writer 链
2. bitrot frame 是 fs3 自定义格式，不是 MinIO 的真实格式
3. `fdatasync` / direct I/O 语义未实现
4. 多盘 write quorum / rename quorum / delete quorum 未实现
5. MinIO 远端盘 RPC 路径对应的最终写盘语义未实现

因此，这一层目前应视为：

1. 已经开始出现 shard 写入
2. 但仍远未达到 MinIO 的真实存储兼容
...
## 3.6.1 两阶段兼容测试视角下，当前存储格式仍然高风险

如果按 `tests/boto3_2tier` 的目标去看当前实现，Phase 1 的“fs3 自写自读”可能部分可工作，但 Phase 2 的“跨实现读取彼此存储”仍然存在非常高的失败风险。

核心原因不是局部小 bug，而是存储格式仍未对齐：

1. fs3 当前 shard 路径结构不是 MinIO 实际结构
2. fs3 当前 bitrot frame 不是 MinIO 实际格式
3. `xl.meta` 中的 erasure 相关字段目前只是近似值，不是 MinIO 真实写盘语义
4. 读路径只会拼 data shards，不能像 MinIO 那样在缺 shard / 坏 shard 时恢复

因此，当前不能把“已出现 erasure/shard/bitrot/quorum 骨架”误判为“已经能通过两阶段存储兼容测试”。
...
- “流式入口 + 验证骨架 + 临时写入 + rename 提交 + 初步 erasure/shard/bitrot/quorum 写路径”
...
4. MinIO-compatible erasure layout
5. MinIO-compatible bitrot format
6. 真实多盘 quorum 语义
7. 读路径 shard 恢复
8. 完整版本事务
9. 读路径元数据语义对齐
...
1. 让当前“流式入口 + 验证骨架 + rename 提交”分支先编译通过并稳定下来
2. 补齐 `Content-MD5` / checksum / aws-chunked / trailer / SSE 的剩余闭环
...
1. 让当前 erasure/shard/bitrot/quorum 骨架对齐 MinIO 的真实落盘格式
2. 实现读路径的缺 shard / 坏 shard 恢复
3. 再继续扩展到多盘/多节点语义
4. 再把 quorum 提升为真实存储提交语义
...
5. `s3_handler` 内部已开始接入请求验证骨架
6. object layer 写路径已开始接入 erasure/shard/bitrot/quorum 骨架
...
3. 流式校验链和 erasure 写路径都已经开始出现
4. 但它们仍未对齐 MinIO 的真实存储兼容语义
...
这部分已经开始出现，但还没有达到 MinIO 兼容级别。

当前后续仍需要支持：

1. MinIO-compatible shard 布局
2. 每 shard 更接近 MinIO 的独立 writer 语义
3. 每 shard 更接近 MinIO 的元数据和校验信息
4. 缺 shard / 坏 shard 时的恢复语义

### 7.4.3 bitrot 相关抽象

这部分也已经开始出现，但当前只是 fs3 自定义 frame，不是 MinIO 真实格式。

如果目标是 MinIO 存储兼容，最终需要一种方式表达：

1. 写入时附带与 MinIO 对齐的校验块
2. 读取时校验块可验证
3. 格式可被 MinIO 读取或至少在两阶段兼容测试中保持一致

### 7.4.4 quorum 语义

当前已经出现最小化的 write quorum 骨架，但这还不是 MinIO 的 quorum 语义。
...
1. 补 `Content-MD5` / checksum / aws-chunked / trailer / SSE 的剩余流式闭环
2. 扩展 `ObjectOptions`
3. 为 retention / checksum / encryption / precondition 预留稳定接口

### 7.5.4 第四阶段

1. 让当前 shard 写入、bitrot、quorum 从“骨架”升级为“MinIO-compatible 语义”
2. 实现读路径恢复
3. 继续推进到多盘/多节点语义
...
fs3 当前最大的问题已经不再是“完全没有提交模型”，也不再是“完全没有 erasure/shard/bitrot/quorum”。

当前真正的问题是：

1. 已经开始出现正确的提交骨架
2. 已经开始出现流式校验骨架
3. 已经开始出现 erasure/shard/bitrot/quorum 写路径骨架
4. 但这些能力目前仍然没有对齐 MinIO 的真实存储兼容语义
5. 当前最重要的是把这些“骨架能力”继续收敛成 MinIO-compatible 实现，而不是误判为已经兼容
