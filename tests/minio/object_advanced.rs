...
async fn object_advanced_test() {
    ...
    // 上传对象
    client.put_object_content(bucket, key, ObjectContent::from(data.as_ref()))
        .send().await.unwrap();

    // 对象标签
    let mut tags = HashMap::new();
    tags.insert("env".to_string(), "test".to_string());
    client.put_object_tagging(bucket, key).tags(tags).send().await.unwrap();
    let got_tags = client.get_object_tagging(bucket, key).send().await.unwrap();
    assert!(!got_tags.tags.is_empty());

    // 删除标签
    client.delete_object_tagging(bucket, key).send().await.unwrap();
