...
async fn policy_engine_test() {
    ...
    client.put_bucket_policy(bucket).config(allow_policy.to_string()).send().await.unwrap();
    ...
    client.put_bucket_policy(bucket).config(multi_stmt_policy.to_string()).send().await.unwrap();
    let got_policy = client.get_bucket_policy(bucket).send().await.unwrap();
    ...
    let result = client.put_bucket_policy(bucket).config(invalid_policy.to_string()).send().await;
