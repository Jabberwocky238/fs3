...
async fn bucket_test() {
    ...
    // get bucket location
    let _location = client.get_bucket_location().bucket(bucket).send().await.unwrap();
    ...
    client.put_bucket_tagging().bucket(bucket).tagging(
        aws_sdk_s3::types::Tagging::builder().tag_set(tag).build().unwrap()
    ).send().await.unwrap();
    let got_tags = client.get_bucket_tagging().bucket(bucket).send().await.unwrap();
    assert!(!got_tags.tag_set().is_empty());
    client.delete_bucket_tagging().bucket(bucket).send().await.unwrap();
    ...
    let ver = client.get_bucket_versioning().bucket(bucket).send().await.unwrap();
    ...
    let _notif = client.get_bucket_notification_configuration().bucket(bucket).send().await.unwrap();
