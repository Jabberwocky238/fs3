...
async fn test_get_object_if_match() {
    ...
    client.put_object().bucket(bucket).key("test.txt").body(ByteStream::from_static(b"hello")).send().await.unwrap();

    let head = client.head_object().bucket(bucket).key("test.txt").send().await.unwrap();
